...
class Solution(Scene):
    def construct(self):
        ...
        eq5_1 = MathTex(r"1 = A(y + x) + By").move_to(ORIGIN + UP)
        eq5_2 = MathTex(r"1 = Ay + Ax + By").move_to(ORIGIN)
        eq5_3 = MathTex(r"1 = (A + B)y + Ax").move_to(ORIGIN + DOWN)
        
        self.play(ReplacementTransform(eq4, eq5_1), ReplacementTransform(step4, step5))
        self.wait(2)
        self.play(Write(eq5_2))
        self.wait(2)
        self.play(Write(eq5_3))
        self.wait(2)

        # Step 5 - Continued
        eq5_4 = MathTex(r"A + B = 0, \quad Ax = 1").move_to(ORIGIN + UP)
        eq5_5 = MathTex(r"A = \frac{1}{x}, \quad B = -\frac{1}{x}").move_to(ORIGIN + DOWN)
        
        self.play(ReplacementTransform(eq5_1, eq5_4), ReplacementTransform(eq5_2, eq5_4), ReplacementTransform(eq5_3, eq5_4))
        self.wait(2)
        self.play(Write(eq5_5))
        self.wait(2)

        # Step 6
        step6 = MathTex(r"\text{Substitute back into the integral:}").to_edge(UP)
        eq6 = MathTex(r"\int \left(\frac{1}{x} \cdot \frac{1}{y} - \frac{1}{x} \cdot \frac{1}{y + x}\right) dy = \int \frac{dx}{x^2}").move_to(ORIGIN)
        
        self.play(ReplacementTransform(eq5_4, eq6), ReplacementTransform(eq5_5, eq6), ReplacementTransform(step5, step6))
        self.wait(2)

        # Step 7
        step7 = MathTex(r"\text{Integrate both sides:}").to_edge(UP)
